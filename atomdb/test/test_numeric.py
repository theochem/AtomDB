...
def test_numerical_hf_data_h():
    ...
    assert_almost_equal(sp.rs[:3], [0.0, 0.183156388887342e-01, 0.194968961085980e-01], decimal=10)
    assert_almost_equal(
        sp.rs[-3:], [0.292242837812349e2, 0.311090881509677e2, 0.331154519586923e2], decimal=10
    )
    ...
    assert_almost_equal(answer[-2:], [0.0, 0.0], decimal=10)
    # check array elements
...
def test_numerical_hf_data_h_anion():
    ...
    # check array elements
    assert_equal(sp.rs >= 0.0, [True] * 139)
    assert_equal(sp.dens_tot >= 0.0, [True] * 139)
    ...
    assert_almost_equal(answer[-20:], [0.0] * 20, decimal=10)
    # check array elements
