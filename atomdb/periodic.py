...
def get_data():
    ...
    for row in data:
        ...
        row[:2] = (int(i) if i else None for i in row[:2])
        row[2:] = (float(i) if i else None for i in row[2:])
...
def get_info():
    ...
    prop2url = {}
    prop2note = {}
    for prop, name, key, unit, desc, src, url, note in info:
        prop2name[prop] = name
        prop2desc.setdefault(prop, {})[key] = desc
        prop2unit.setdefault(prop, {})[key] = unit
        prop2src.setdefault(prop, {})[key] = src
        prop2url.setdefault(prop, {})[key] = url
        prop2note.setdefault(prop, {})[key] = note
    return prop2name, prop2desc, prop2unit, prop2src, prop2url, prop2note
