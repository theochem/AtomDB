...
DATASET_PROPERTIES = {
    ...
    "elem": {
        "TYPE": "GENERAL",
        "DESC": "Refers to an element from the Periodic Table."
    },
    "dataset": {
        "TYPE": "GENERAL",
    ...
    "mass": {
        "TYPE": "SCALAR",
        "DESC": "Atomic mass of the element."
    },
    "energy": {
        "TYPE": "SCALAR",
    ...
    "_": {
        "TYPE": "NA",
        "DESC": "Property Information Not Found"
    }
}
        ...
        if len(props) > 0:
            if doctype == "DOCSTRING":
                ...
                for prop in props:
                    attr = DATASET_PROPERTIES.get(prop, DATASET_PROPERTIES["_"])
                    docs += ' '.join(
                        f"> \033[1;32;40m{prop}\033[0m ({attr['TYPE']}): {attr['DESC']}".split('.')[:n] \
                            + ['\n']
                    )
            else:
